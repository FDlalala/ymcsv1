"""
eval_local_parallel.py —— 针对 rag_local.py（RetrievalQA 链）的 8 卡并行评估主控
功能：
  1. 将 40 道题均匀分配给 8 个 GPU（每卡 5 题）
  2. 启动 8 个 eval_local_worker.py 进程，每个绑定一张 GPU
  3. 实时打印每个进程的输出（带 Worker 前缀）
  4. 全部完成后合并分片 JSON → 完整 JSON + 汇总报告

用法：
  cd /apdcephfs_cq8/share_2906397/users/futurelwei/hw/rag
  python eval_local_parallel.py [--num_gpus 8] [--save_dir ./eval_results_local]
"""
...
print(f"[rag_local 评估] 共 {total} 道题，分配给 {NUM_GPUS} 张 GPU")
...
print(f"\n{'='*60}")
print(f"启动 {NUM_GPUS} 个并行 Worker（rag_local 模式）...")
print(f"{'='*60}\n")
...
for shard_id in range(NUM_GPUS):
    ...
    cmd = [
        sys.executable, "eval_local_worker.py",
...
# 写入合并后的完整 JSON
merged_json = os.path.join(SAVE_DIR, f"eval_local_detail_{timestamp}.json")
...
report_path = os.path.join(SAVE_DIR, f"eval_local_report_{timestamp}.txt")
with open(report_path, "w", encoding="utf-8") as f:
    f.write("=" * 70 + "\n")
    f.write(f"rag_local（RetrievalQA 链）评估报告  生成时间: {timestamp}\n")
    ...
    f.write("各类别统计:\n")
    f.write(f"  {'类别':<25} {'题数':>4} {'OK':>4} {'OK率':>7} {'平均耗时':>8}  错误分布\n")
    f.write("  " + "-" * 65 + "\n")
    for cat, stat in category_stats.items():
        f.write(f"  {cat:<25} {stat['total']:>4} {stat['ok']:>4} "
                f"{stat['ok_rate']*100:>6.1f}% {stat['avg_elapsed']:>7.1f}s  "
                f"{stat['error_dist']}\n")
    f.write("\n")
...
# ============================================================
print(f"\n{'='*60}")
print("rag_local 评估完成！总体统计：")
print(f"{'='*60}")
print(f"  {'类别':<25} {'OK率':>7}  错误分布")
print("  " + "-" * 60)
for cat, stat in category_stats.items():
    print(f"  {cat:<25} {stat['ok_rate']*100:>6.1f}%  {stat['error_dist']}")
